(function($){
    ...
    function s7upf_qty_click(){
        ...
        $("body").on("click",".quantity .qty-down",function(){
            ...
            var step = seff.attr("step");
            if(step === undefined) step = 1;
            if(Number($(this).prev('input').val()) > 1){
	...
	$(document).ready(function(){
		...
		fix_variable_product();
            ...
            $( '.list-select-attr select' ).change( function() {
               var val = $(this).val();
               var data_select = $(this).next().attr('data-select');
                data_select =JSON.parse(data_select);
                if(!val){
                    location.href = data_select['none'];
                }else{
                    location.href = data_select[val];
                }
        ...
        $('.mb-item-variations').find('.tawcvs-swatches .swatch').on('click',function () {
            var data = $(this).find('.hide').html();
            $(this).parents('.mb-item-variations').find('.mb-name-value').html(data);
        })

        $('.mb-item-variations').find('select').on('change', function() {
            var sel = $(this);
            var data = sel[0].children[sel[0].selectedIndex].text;
            sel.parents('.mb-item-variations').find('.mb-name-value').html(data);
            console.log(data);
        })
		//Fix mailchimp
	...
	$(window).load(function(){
        ...
        if($('.rtl-enable').length > 0){
            $('*[data-vc-full-width="true"]').each(function(){
                ...
                style = style.replace(" left: -"," right: -");
                style = style.replace("--","-");
                $(this).attr('style',style+'left: inherit');
            })
        }
