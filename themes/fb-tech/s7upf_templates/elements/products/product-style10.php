...
if(!empty($data_item_tab) and is_array($data_item_tab)){ ?>
    <div class="woocommerce mb-element-product-<?php echo esc_attr($style); ?> <?php echo esc_attr($extra_class)?>">
        ...
        <div class="product-tab-box">
            <ul class="title-best-tab list-inline-block text-center">
                <?php foreach ($data_item_tab as $key => $value){
                    if($key +1 == (int)$position_tab_active) $class_active = 'active'; else $class_active =''; ?>
                    <li class="<?php echo esc_attr($class_active); ?>"><a class="shop-button" href="#tab-vip<?php echo esc_attr($key); echo esc_attr($time); ?>" data-toggle="tab"><?php if(!empty($value['title'])) echo esc_attr($value['title']); ?></a></li>
                <?php } ?>
            </ul>
        </div>
                ...
                <div id="tab-vip<?php echo esc_attr($key); echo esc_attr($time); ?>" class="tab-pane fade in <?php echo esc_attr($class_active); ?>">
                    <div class="product-tab-slider">
                        <div class="wrap-item group-navi center-navi" data-pagination="<?php echo esc_attr($pagination_slider); ?>" data-navigation="<?php echo esc_attr($navigation)?>" data-autoplay="<?php echo esc_attr($autoplay); ?>" data-itemscustom="[<?php echo esc_attr($itemscustom)?>]">
                            ...
                            $j=1; $k=1; $class_bg_img='';
                            if($vip_query->have_posts()) {
                                while($vip_query->have_posts()) {
                                    $vip_query->the_post(); global $product; ;
                                    if($j %2 == 1 ){
                                        if($i%2==0){
                                    ...
                                    <div class="item-product text-center boxed style1">
                                    ...
                                    <?php if($i % (int)$number_row == 0 || $i == $count_post  || $number_row == 1) echo '</div>';  $i=$i+1; if($i % (int)$number_row == 1 ) $j++;?>
                                    <!-- End Item -->
