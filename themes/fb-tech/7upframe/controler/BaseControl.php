...
if(!class_exists('S7upf_BaseController'))
{
    class S7upf_BaseController
    {
        ...
        static function _add_scripts()
        {
            ...
            wp_enqueue_script( 'jquery.fancybox.min',$js_url.'libs/jquery.fancybox.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'jquery-ui.min',$js_url.'libs/jquery-ui.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'owl.carousel.min',$js_url.'libs/owl.carousel.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'jquery.jcarousellite.min',$js_url.'libs/jquery.jcarousellite.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'jquery.elevatezoom.min',$js_url.'libs/jquery.elevatezoom.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'popup.min',$js_url.'libs/popup.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'timecircles.min',$js_url.'libs/timecircles.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'wow.min',$js_url.'libs/wow.min.js',array('jquery'),null,true);
            wp_enqueue_script( 'slick.min',$js_url.'libs/slick.min.js',array('jquery'),null,true);
            ...
            // CSS
            $font_url = add_query_arg( 'family', urlencode( 'Open Sans:300,400,700|Poiret One|Anton'), "//fonts.googleapis.com/css" );
            wp_enqueue_style( 's7upf-Open-Sans', $font_url, array(), null );
            wp_enqueue_style('ionicons.min',$css_url.'libs/ionicons.min.css');
            ...
            wp_enqueue_style('jquery.fancybox.min',$css_url.'libs/jquery.fancybox.min.css');
            wp_enqueue_style('jquery-ui.min',$css_url.'libs/jquery-ui.min.css');
            wp_enqueue_style('owl.carousel.min',$css_url.'libs/owl.carousel.min.css');
            wp_enqueue_style('owl.transitions.min',$css_url.'libs/owl.transitions.min.css');
            wp_enqueue_style('jquery.mCustomScrollbar.min',$css_url.'libs/jquery.mCustomScrollbar.min.css');
            wp_enqueue_style('owl.theme.min',$css_url.'libs/owl.theme.min.css');
            wp_enqueue_style('animate.min',$css_url.'libs/animate.min.css');
            wp_enqueue_style('hover.min',$css_url.'libs/hover.min.css');
            wp_enqueue_style('slick.min',$css_url.'libs/slick.min.css');
        ...
        static function s7upf_body_classes($classes) {
            ...
            if('on' === $rtl_check || is_rtl()) $classes[] = 'rtl-enable';
            $layout_boxed = s7upf_get_value_by_id('s7upf_boxed_width');
            if('on' === $layout_boxed) $classes[] = 'padding-box';
            $margin_boxed = s7upf_get_value_by_id('s7upf_margin_boxed');
            if(!empty($margin_boxed) and 'on' === $layout_boxed  ){
                $classes[] = S7upf_Assets::build_css('margin:'.$margin_boxed.';','.padding-box .wrap');
                ...
                foreach($add_sidebars as $sidebar){
                    if(!empty($sidebar['title'])){
                        ...
                        $custom_add_sidebar = array(
                                'name' => $sidebar['title'],
                                'id' => $id,
                                'description' => esc_html__( 'SideBar created by add sidebar in theme options.', 'fb-tech'),
        ...
        static function _add_admin_scripts()
        {
            ...
            // import icon ios
            wp_enqueue_style( 's7upf-ionicons.min',get_template_directory_uri().'/assets/css/libs/ionicons.min.css');
